...
本 Skill 自带若干脚本，用来保持仓库整洁、落实标准。你想自己得到和 Agent 相同的结果时，可以手动运行。

| 脚本 | 用途 |
|------|------|
| `scripts/audit_style.py <path>` | 一致性探针（覆盖 `LHT-/HY-/PL-/GP-*` 的子集）：输出固定的发现表。 |
| `scripts/sync_gitignore.py [path]` | 根据当前目录结构自动维护 `.gitignore`。在带标记的自动管理块内推导忽略条目；手写规则被保留。CI 中用 `--check`。 |
| `scripts/run_gate.sh` / `scripts/run_gate.ps1` | 运行强制质量门（`black`/`isort`/`ruff`/`mypy`/`pytest`），并把所有工具缓存重定向进 `.cache/`。 |

### 缓存收口，而非散落

与项目代码**无直接关联**的工具缓存与运行产物，统一收进仓库根的一个 `.cache/` 文件夹
（`.mypy_cache/`、`.pytest_cache/`、`.ruff_cache/`、`.coverage`、`htmlcov/`）。
Hydra 的运行输出（`logs/`、`outputs/`、`wandb/`）作为真实实验产物保留在根目录。
`.cache/` 下的内容都已被 `.gitignore` 覆盖，绝不提交 —— 见 `.cache/README.md`。

### 自动维护的 .gitignore

在搭建（场景 A）或整理（场景 B）之后，Agent 会运行 `python scripts/sync_gitignore.py .`，
使忽略列表始终跟踪当前布局 —— 新增的产物目录（`logs/`、`wandb/`、`checkpoints/`……）自动被忽略，无需手工编辑。
...
随后运行一致性探针与自动 `.gitignore` 同步：

```bash
python scripts/audit_style.py .            # 一致性发现表
python scripts/sync_gitignore.py .         # 让 .gitignore 与布局保持同步
bash scripts/run_gate.sh                   # 完整质量门，缓存 -> .cache/
```
